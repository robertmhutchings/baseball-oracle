...
- **Phase 2 ingest detail**: the multi-year (1901–2023) ingest plan — quirks-at-scale catalog, per-year transaction strategy, verification layers, and open questions — lives in `ingest/phase2_plan.md`. Consult before writing or modifying any Phase 2 ingest code.
...
> **Phase 1 SQL validation complete.** All five initial benchmark queries verified end-to-end. The data layer (`retro` schema, 1998 load) is trustworthy for player-level, game-level, team-level, and narrative queries. Proceeding to Phase 2 (full 1901–2023 ingest) or Phase 3 (agent layer) based on direction from PM.
...
- **Provenance.** Users want to know the data source. We can state "all statistics from Retrosheet 1901–2023" on the site and that claim must remain literally true.
...
├── .gitignore
├── CLAUDE.md                # this file
├── data\                    # Retrosheet CSVs (1901-2023 year folders + cross-year files)
├── schema\
